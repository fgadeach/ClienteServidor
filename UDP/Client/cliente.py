...
if __name__ == '__main__':
    ...
    with open(f"Recibidos/{numeroClientes}–Prueba-{numeroConexiones}.txt", "w") as f:
        while True:
            datos, direccion = cliente.recvfrom(1024)
    ...
    tamanioArchivoRecibido = os.path.getsize(f"Recibidos/{numeroClientes}–Prueba-{numeroConexiones}.txt")

    hash = hashlib.md5()
    with open(f"Recibidos/{numeroClientes}–Prueba-{numeroConexiones}.txt", 'rb') as f:
        while True:
            datos = f.read(1024)
            if not datos:
                break
